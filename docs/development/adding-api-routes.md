...
API routes are defined in `cascadia-api/src/server/routes/`, one file per domain. Each file creates a `Hono` app, defines routes using `adapt()` + `apiHandler()`, and exports the app. The routes are mounted in `cascadia-api/src/server/index.ts`.

| File                                              | Mounted At              |
| ------------------------------------------------- | ----------------------- |
| `cascadia-api/src/server/routes/parts.ts`         | `/api/v1/parts`         |
| `cascadia-api/src/server/routes/programs.ts`      | `/api/v1/programs`      |
| `cascadia-api/src/server/routes/designs.ts`       | `/api/v1/designs`       |
| `cascadia-api/src/server/routes/change-orders.ts` | `/api/v1/change-orders` |
...
`adapt()` from `cascadia-api/src/server/adapter.ts` bridges Hono's `Context` to the `apiHandler()` signature. It extracts `params` and `request` from the Hono context and passes them to the legacy handler:
...
After creating a new route file, you must import and mount it in `cascadia-api/src/server/index.ts`:
