...
The exemption is declared in `cascadia-api/src/items/branch-protection.ts`, alongside `ChangeOrder`, and covers **branch protection only** -- a work instruction another user holds checked out is still locked against you.
...
| Area                        | Path                                                                   |
| --------------------------- | ---------------------------------------------------------------------- |
| Database schema             | `cascadia-api/src/db/schema/items.ts` (search for `workInstructions`)  |
| Type definitions            | `cascadia-commons/src/items/types/work-instruction.ts`                 |
| Branch-protection exemption | `cascadia-api/src/items/branch-protection.ts`                          |
| Output part picker (UI)     | `cascadia-web/src/components/work-instructions/OutputPartField.tsx`    |
| Output part invariants      | `cascadia-api/src/server/routes/work-instructions.output-part.test.ts` |
| Item type registration      | `cascadia-api/src/items/registerItemTypes.server.ts`                   |
| Numbering scheme            | `cascadia-commons/src/items/numbering/schemes.ts`                      |
| Inheritance service         | `cascadia-api/src/services/WorkInstructionInheritanceService.ts`       |
| Change alert service        | `cascadia-api/src/services/WorkInstructionChangeAlertService.ts`       |
| Traveler service            | `cascadia-api/src/services/WorkOrderInstructionService.ts`             |
| Execution service           | `cascadia-api/src/services/InstructionExecutionService.ts`             |
| Traveler/execution schema   | `cascadia-api/src/db/schema/work-orders.ts`                            |
| Parametric resolution       | `cascadia-api/src/services/ParametricResolutionService.ts`             |
| Background job definitions  | `cascadia-api/src/jobs/definitions/workinstruction/`                   |
| API routes                  | `cascadia-api/src/server/routes/work-instructions.ts`                  |
| Parts reverse-lookup        | `cascadia-api/src/server/routes/parts.ts`                              |
| UI components               | `cascadia-web/src/components/work-instructions/`                       |
