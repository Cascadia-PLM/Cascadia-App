...
The vault is implemented primarily in `cascadia-api/src/vault/`, with the `FileService` class providing the service layer and the `VaultStorage` interface abstracting the storage backend.
...
The vocabulary lives in `cascadia-commons/src/vault/preview.ts` and is shared by the server
...
Vocabulary lives in `cascadia-commons/src/vault/annotations.ts`, shared by the viewer, the
...
The vocabulary lives in `cascadia-commons/src/vault/file-categories.ts` — labels, badge
...
shared from `cascadia-commons/src/vault/image-files.ts` — as a grid of tiles, and opens any
...
| File                                                         | Purpose                                                                   |
| ------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `cascadia-api/src/vault/services/FileService.ts`             | Core service: upload, download, checkout, checkin, versioning, listing    |
| `cascadia-api/src/vault/storage/types.ts`                    | `VaultStorage` interface and configuration types                          |
| `cascadia-api/src/vault/storage/local-storage.ts`            | Local filesystem storage implementation                                   |
| `cascadia-api/src/vault/storage/s3-storage.ts`               | S3-compatible storage implementation                                      |
| `cascadia-api/src/vault/storage/storage-factory.ts`          | Factory for creating storage instances from config                        |
| `cascadia-api/src/vault/utils/file-utils.ts`                 | File validation, hashing, category detection, path generation             |
| `cascadia-commons/src/vault/file-categories.ts`              | Shared category vocabulary: labels, badges, filter options                |
| `cascadia-commons/src/vault/preview.ts`                      | Shared preview allowlist: which formats render in-app, and as what        |
| `cascadia-api/src/db/schema/vault.ts`                        | Database schema: `vault_files` and `vault_file_history` tables            |
| `cascadia-web/src/components/vault/FileList.tsx`             | UI component: file listing with lock status and actions                   |
| `cascadia-web/src/components/vault/PdfViewer.tsx`            | UI component: pdf.js viewer (code-split; not exported from the barrel)    |
| `cascadia-web/src/components/vault/FilePreview.tsx`          | UI component: fetches bytes and mounts the viewer for the format          |
| `cascadia-web/src/components/vault/SvgViewer.tsx`            | UI component: vector drawing viewer — zoom, pan, rotate, fullscreen       |
| `cascadia-web/src/components/vault/viewer-controls.ts`       | Zoom stops and fullscreen state shared by the PDF and SVG viewers         |
| `cascadia-web/src/components/vault/FilePreviewDialog.tsx`    | UI component: preview one file in a dialog, opened from `FileList`        |
| `cascadia-web/src/components/vault/ItemFilePreviewPanel.tsx` | UI component: in-page preview of an item's files (Documents' Preview tab) |
| `cascadia-web/src/components/vault/PdfAnnotationLayer.tsx`   | UI component: the markup surface over one rendered PDF page               |
| `cascadia-web/src/components/vault/useFileMarkup.tsx`        | Markup state, mutations, and the comment/label prompt                     |
| `cascadia-web/src/components/vault/FileCategoryMenu.tsx`     | UI component: category picker for correcting a file's category            |
| `cascadia-web/src/components/vault/FileUploadZone.tsx`       | UI component: drag-and-drop file upload                                   |
| `cascadia-api/src/server/routes/files.ts`                    | API route handlers for all file operations                                |
| `cascadia-api/src/server/routes/items.ts`                    | API route handlers for item-scoped file operations                        |
