...
The `PermissionService` (singleton at `cascadia-api/src/auth/permission-service.ts`) handles all permission checks:
...
The `AccessControlService` (`cascadia-api/src/auth/AccessControlService.ts`) enforces program isolation:
...
Two convenience functions in `cascadia-api/src/auth/access.ts` enforce design and branch access:
...
`cascadia-api/src/items/item-type-resources.ts`, a test keeps it total,
