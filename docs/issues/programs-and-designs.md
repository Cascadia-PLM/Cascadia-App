...
**Severity:** Low
**File:** `packages/core/src/lib/services/ProgramService.ts` (lines 194-207)
...
**Severity:** Low
**File:** `packages/core/src/lib/services/ProgramService.ts` (lines 143-177)
...
**File:** `packages/core/src/server/routes/designs.ts`

The `DELETE /api/v1/designs/:id` endpoint calls `DesignService.archive()` which sets `isArchived=true` rather than actually deleting the design. This is correct behavior (soft delete) but may surprise API consumers who expect DELETE to be destructive. The API response (`{ success: true }`) does not indicate that this was an archive operation.
...
**Severity:** Low
**File:** `packages/core/src/lib/services/DesignService.ts` (line 305)
...
**Severity:** Low
**File:** `packages/core/src/lib/db/schema/crossReferences.ts` (lines 72-76)
...
**Severity:** Low
**File:** `packages/core/src/server/routes/programs.ts`
