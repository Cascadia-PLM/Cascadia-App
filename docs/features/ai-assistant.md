...
- **Component**: `ChatPanelButton` (`cascadia-web/src/components/ai/ChatPanelButton.tsx`)
- **State management**: `ChatPanelProvider` context (`cascadia-web/src/ai/chat-context.tsx`)
...
Three tables support AI chat persistence (defined in `cascadia-api/src/db/schema/ai.ts`):
...
The `ConfirmationCard` (`cascadia-web/src/components/ai/ConfirmationCard.tsx`) displays:
...
Defaults live in `DEFAULT_MODEL` in `cascadia-commons/src/ai/model-catalog.ts` and are re-exported as `DEFAULT_MODELS` from `adapters.ts`, so there is one place to change them.

### Model Discovery

The admin model picker is populated at runtime rather than from a hardcoded list. `listProviderModels()` in `cascadia-api/src/ai/model-discovery.ts` queries each provider's own list-models endpoint, and `POST /api/v1/admin/ai-settings/models` exposes it to the UI:
...
The `getAdapter()` function in `cascadia-api/src/ai/adapters.ts` creates the appropriate TanStack AI adapter based on the provider configuration. It accepts a provider type, model name, API key, and optional base URL.
...
**Security**: API keys stored in the database are encrypted at rest using the `@/crypto/encryption` module. The GET endpoint masks API keys in responses (returns `***` instead of the actual key). Keys with known provider prefixes (`sk-`, `key-`) are detected as plaintext and skipped during decryption.
...
Tools are assembled in `cascadia-api/src/ai/tools/index.ts`:
...
The `KnowledgeService` (`cascadia-api/src/ai/KnowledgeService.ts`) makes the AI schema-aware by:
...
Every tool handler is wrapped with `withPermissionAndAudit()` (for read tools) or `withWritePermissionAndAudit()` (for write tools) from `cascadia-api/src/ai/tools/permission-wrapper.ts`. These wrappers:
...
Server code lives in `cascadia-api/src/items/enrichment/` (`enrich-item.ts`, `fetch-source.ts`, `html-to-text.ts`, `limits.ts`); the client side in `cascadia-web/src/components/items/` (`useDropEnrichment.ts`, `enrichment-sources.ts`, `image-payload.ts`, `apply-enrichment.ts`, `DropOverlay.tsx`, `PendingImageStrip.tsx`).
...
| File                                                  | Purpose                                                      |
| ----------------------------------------------------- | ------------------------------------------------------------ |
| `cascadia-api/src/ai/adapters.ts`                     | Provider adapter factory and config loading                  |
| `cascadia-api/src/ai/SessionService.ts`               | Session and message persistence                              |
| `cascadia-api/src/ai/KnowledgeService.ts`             | Schema introspection and system prompt generation            |
| `cascadia-web/src/ai/chat-context.tsx`                | React context for panel state management                     |
| `cascadia-api/src/ai/tools/definitions.ts`            | Read-only tool definitions (Zod schemas)                     |
| `cascadia-api/src/ai/tools/write-definitions.ts`      | Write tool definitions with confirmation schemas             |
| `cascadia-api/src/ai/tools/handlers.ts`               | Read-only tool handler implementations                       |
| `cascadia-api/src/ai/tools/write-handlers.ts`         | Write tool handler implementations                           |
| `cascadia-api/src/ai/tools/permission-wrapper.ts`     | Permission checking and audit logging wrapper                |
| `cascadia-api/src/ai/tools/index.ts`                  | Tool assembly and exports                                    |
| `cascadia-api/src/db/schema/ai.ts`                    | Database schema for sessions, messages, settings, usage logs |
| `cascadia-web/src/components/ai/ChatPanel.tsx`        | Main chat sidebar component                                  |
| `cascadia-web/src/components/ai/ChatMessage.tsx`      | Message rendering with Markdown and tool results             |
| `cascadia-web/src/components/ai/ChatInput.tsx`        | Input component with Send/Search modes                       |
| `cascadia-web/src/components/ai/ChatPanelButton.tsx`  | Edge button to open the panel                                |
| `cascadia-web/src/components/ai/ConfirmationCard.tsx` | Confirmation UI for write operations                         |
| `cascadia-api/src/server/routes/ai.ts`                | Chat, session, message history, and settings endpoints       |
