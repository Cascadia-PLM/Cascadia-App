...
**The 13 types:** Part, Document, ChangeOrder, Requirement, Task, WorkInstruction, Issue, TestPlan, TestCase, Software, Tool, PhysicalPart, WorkOrder. The canonical list lives in `cascadia-commons/src/items/item-type-definitions.ts`.
...
Each item type has a default numbering scheme defined in `cascadia-commons/src/items/numbering/schemes.ts`:
...
- Schema: `cascadia-api/src/db/schema/items.ts` (parts table)
- Types: `cascadia-commons/src/items/types/part.ts`
...
- Schema: `cascadia-api/src/db/schema/items.ts` (documents table)
- Types: `cascadia-commons/src/items/types/document.ts`
- Form: `cascadia-web/src/components/documents/DocumentForm.tsx`
- Vault schema: `cascadia-api/src/db/schema/vault.ts`
...
- Schema: `cascadia-api/src/db/schema/items.ts` (change_orders and related tables)
- Types: `cascadia-commons/src/items/types/change-order.ts`
- Form: `cascadia-web/src/components/change-orders/ChangeOrderForm.tsx`
- Service: `cascadia-api/src/items/services/ChangeOrderService.ts`
- Release: `cascadia-api/src/services/ChangeOrderMergeService.ts` (in service layer)
...
- Schema: `cascadia-api/src/db/schema/items.ts` (requirements table)
- Types: `cascadia-commons/src/items/types/requirement.ts`
...
- Schema: `cascadia-api/src/db/schema/items.ts` (tasks table)
- Types: `cascadia-commons/src/items/types/task.ts`
...
- Schema: `cascadia-api/src/db/schema/items.ts` (work_instructions and related tables)
- Types: `cascadia-commons/src/items/types/work-instruction.ts`
...
- Schema: `cascadia-api/src/db/schema/items.ts` (issues table)
- Types: `cascadia-commons/src/items/types/issue.ts`
...
- Program schema: `cascadia-api/src/db/schema/programs.ts`
- Design schema: `cascadia-api/src/db/schema/designs.ts`
- Versioning schema: `cascadia-api/src/db/schema/versioning.ts` (branches, commits, tags)
...
1. **Code definitions** -- everything about a type: its schema, table, labels, icon, relationships and searchable fields. They live in `ITEM_TYPE_DEFINITIONS` (`cascadia-commons/src/items/item-type-definitions.ts`) and are registered by `registerItemTypes.server.ts`.
...
`cascadia-commons/src/items/item-type-definitions.ts` holds the definitions;
...
Each item type is assigned a lifecycle definition by ID (defaults in `cascadia-commons/src/items/lifecycle-ids.ts`). The lifecycle controls valid states and transition rules. Multiple item types can share the same lifecycle definition. An administrator can reassign it under **Admin > Item Types**; the swap is refused if the target is the wrong kind of definition or if any existing item is in a state the target does not define.

### Permissions Model

Access is not configured per item type. Each type maps to an RBAC resource
(`ITEM_TYPE_RESOURCES` in `cascadia-api/src/items/item-type-resources.ts`
...
to its source (see `items/traceability-relationships.ts`).
