...
/**
 * Byte order, not locale order. The canonical form feeds the unique index on
 * `item_relationships.option` and the conflict signature, so two runtimes
 * with different ICU data must still agree on it.
 */
...
export function normalizeOptionCondition(
  ...
  return {
    all: [...byFamily.entries()]
      ...
      .map(([family, values]) => ({
        family,
        values: [...values].sort(compareCodes),
      })),
  }
}
