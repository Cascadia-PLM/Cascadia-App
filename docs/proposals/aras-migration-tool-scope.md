...
**Status**: Not started. No migration code exists (`src/migration/` and the
`migrate:aras` scripts described here and in the
[implementation plan](./aras-migration-implementation-plan.md) were never built).
This is a design proposal for future work.
...
| --------------- | -------------------------- | --------------------------------- | --------------------------------------------------- |
| ItemType name   | `innovator.ItemType` table | `ItemTypeConfig.name`             | Code: `packages/core/src/lib/items/types/{name}.ts` |
...
| Aras Concept      | Storage in Aras          | Cascadia Equivalent    | Output Location                                          |
| ----------------- | ------------------------ | ---------------------- | -------------------------------------------------------- |
| Form definition   | `innovator.Form` table   | React Form component   | Code: `packages/core/src/lib/items/forms/{Name}Form.tsx` |
...
2. Convert to Cascadia `WorkflowDefinition` with `lifecycleType: 'Driven'`
...
#### 1. Code Generation (`packages/core/src/lib/items/types/`)
...
```
packages/core/src/lib/items/types/
...
1. Copy generated files to Cascadia project:
   - Types → `packages/core/src/lib/items/types/`
   - Forms → `packages/core/src/lib/items/forms/`
   - Tables → `packages/core/src/lib/items/tables/`
2. Run database migrations: `npm run db:generate && npm run db:migrate`
3. Import runtime configs: `npm run migrate:import-configs`
4. Register new types in `packages/core/src/lib/items/index.ts`
