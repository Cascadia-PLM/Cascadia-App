...
Everything lives in `cascadia-api/src/packages/`:
...
```typescript
import { PackageRegistry, requirePackage } from '@cascadia/api/packages'
...
**1. Add the id to the union** in `cascadia-commons/src/packages/types.ts`:

```typescript
export type PackageId = 'advanced-auditing' | 'your-package'
```

**2. Describe it** in `cascadia-api/src/packages/catalog.ts`. The description and feature
...
**3. Put the code under its own directory**, `cascadia-api/src/your-package/`, so the
...
```tsx
import { usePackageEnabled } from '@cascadia/web/hooks/usePackages'
...
| Concern         | Location                                       |
| --------------- | ---------------------------------------------- |
| Services, logic | `cascadia-api/src/your-package/`               |
| Database schema | `cascadia-api/src/db/schema/your-package.ts`   |
...
1. Export the new schema file from `cascadia-api/src/db/schema/index.ts`.
